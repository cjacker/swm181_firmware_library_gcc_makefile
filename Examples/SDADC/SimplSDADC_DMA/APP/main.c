...
int main(void)
{
	...
	DMA_CH_Config(DMA_CHR_SDADC, (uint32_t)SDADC_Result, 1000, 1);
...
void IRQ5_Handler(void)
{
	...
	if(DMA_CH_INTStat(DMA_CHR_SDADC))
	{
		...
		DMA_CH_INTClr(DMA_CHR_SDADC);		//清除中断标志
		
		for(i = 0; i < 1000; i++)
		{
			chn = (SDADC_Result[i] >> 16) & 0x0F;
			res = (int16_t)(SDADC_Result[i] & 0xFFFF);
			printf("%d,", res);
		}
		
		DMA_CH_Open(DMA_CHR_SDADC);			//在传输完成后通道开启会自动清零
		SDADC_Start(SDADC);
	}
}
