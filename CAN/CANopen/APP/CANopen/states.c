...
extern void RtcSetTimer(unsigned int   count);

void Cmd(Message *m)
{
	unsigned char cs=m->data[0];
	unsigned short indexbuf =(((unsigned short)(m->data[2])<<8)|(unsigned short)(m->data[1]));
	if(((cs>>5)==INITIATE_DOWNLOAD_RESPONSE)&&(indexbuf==0x2a18))
				{RTCBatteryTime=0;  }
	
}
 extern UNS32 TestSlave_obj1400_COB_ID_used_by_PDO ;
...
//	unsigned char error;
	 ...
	 switch(cob_id >> 7)//功能码
	{
		...
		case SDOrx:
		//BE 1B 51 52  低位在前，2013年10月6日16时13分50秒
			
				if (d->CurrentCommunicationState.csSDO)
				{
					if(((cs>>5)==INITIATE_UPLOAD_REQUEST)&&(indexbuf==0x2a18))
					{RTCBatteryTime=0;  }//读RTC 时间++
					
					proceedSDO(d,m);  //smile 说：这个留着，不能注释
